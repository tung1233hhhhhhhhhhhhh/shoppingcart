...
if (isset($_POST['btn-reg'])) {
    ...
    if (!empty($fullname) && !empty($sdt) && !empty($password) && !empty($address) && !empty($email) && !empty($gender)) {
        ...
        if ($resultemail['count']>0) {
            ...
            $stmt = $conn->prepare("INSERT INTO tbl_users (fullname, sdt, password, gender, email, address) VALUES (:fullname, :sdt, md5(:password), :gender, :email, :address)");
            ...
            try {
                $stmt->execute();
                $sql = $conn->prepare("SELECT id FROM tbl_users WHERE email = :email");
