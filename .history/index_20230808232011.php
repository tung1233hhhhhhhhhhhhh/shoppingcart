        ...
        <div class="container">
            <?php 
                if (!isset($_SESSION['user'])) { ?>
                    <a href="./view/user/login.php"><button type="button">Đăng nhập</button></a>
                    <a href="./view/user/signup.php"> <button type="button">Đăng KÝ</button></a>
                    <?php }else{ ?>
                     <p>Xin Chào: <?php foreach ($_SESSION['user'] as $user) {
        if (isset($user['name'])) {
            echo $user['name'];
        }
    } ?></p>
